...
async def vps_info_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            url_status = subprocess.check_output("curl -s -o /dev/null -w '%{http_code}' https://console.dashwave.io/workspace/8042", shell=True).decode().strip()
        except Exception:
            url_status = "error"
    ...
    try:
        ...
        try:
            url_check = subprocess.run(
                "curl -s -o /dev/null -w '%{http_code}' https://console.dashwave.io/workspace/8042",
                shell=True,
                capture_output=True,
                text=True
            )
...
async def ping_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        url = "https://console.dashwave.io/workspace/8042"
        if context.args:
            url = context.args[0]
